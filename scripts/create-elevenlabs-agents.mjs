...
/**
 * Creates ElevenLabs Conversational AI agents for all 10 pharmacy scenarios.
 * Reads prompts from agent-prompts.ts, creates agents via API,
 * and updates .env with the returned agent IDs.
 *
 * Usage: node scripts/create-elevenlabs-agents.mjs
 */

const API_KEY = "sk_b934cbf239802c195f499266f8b870b368a3d7301e95d709";
const API_BASE = "https://api.elevenlabs.io/v1/convai/agents/create";

const FIRST_MESSAGE =
  "Wir simulieren gleich ein 1-zu-1 Kundengespräch in einer Apotheke. Sag kurz Bescheid, wenn du bereit bist, in dem du mit der Gesprächseröffnung beginnst.";

// TTS settings from existing diuretikum agent
const TTS_CONFIG = {
  model_id: "eleven_turbo_v2_5",
  voice_id: "M39iqBUcu1jyiwM5PfSy",
...
// ============================================================
// All 10 agent prompts (from data/agent-prompts.ts)
// ============================================================

const agents = [
  {
    key: "pharmacy_diuretikum",
    envVar: "PHARMACY_DIURETIKUM_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_fluorid",
    envVar: "PHARMACY_FLUORID_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_gastritis",
    envVar: "PHARMACY_GASTRITIS_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_gicht",
    envVar: "PHARMACY_GICHT_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_eisenmangel",
    envVar: "PHARMACY_EISENMANGEL_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_grippe",
    envVar: "PHARMACY_GRIPPE_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_helicobacter",
    envVar: "PHARMACY_HELICOBACTER_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_hyperthyreose",
    envVar: "PHARMACY_HYPERTHYREOSE_AGENT_ID",
...
- Sage, du kannst nachts kaum schlafen und bist tagsueber total aufgedreht.
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_hypothyreose",
    envVar: "PHARMACY_HYPOTHYREOSE_AGENT_ID",
...
- Halte deine Antworten kurz (1-3 Saetze).`,
  ...
  {
    key: "pharmacy_kompression",
    envVar: "PHARMACY_KOMPRESSION_AGENT_ID",
...
async function createAgent(agent) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to create ${agent.key}: ${res.status} ${err}`);
  }
...
async function main() {
  console.log("=== ElevenLabs Agent Creator ===\n");
  console.log(`Creating ${agents.length} agents...\n`);

  const results = {};

  for (const agent of agents) {
    process.stdout.write(`  Creating ${agent.key}... `);
    try {
      const agentId = await createAgent(agent);
      results[agent.key] = { envVar: agent.envVar, agentId };
      console.log(`OK -> ${agentId}`);
    } catch (err) {
      console.log(`FAILED: ${err.message}`);
      results[agent.key] = { envVar: agent.envVar, agentId: null, error: err.message };
    }
  }
  ...
  for (const [key, val] of Object.entries(results)) {
    if (!val.agentId) continue;
    const envVar = val.envVar;
    // Replace existing line or add new one
    const regex = new RegExp(`^${envVar}=.*$`, "m");
    const newLine = `${envVar}="${val.agentId}"`;
  ...
  // Summary
  console.log("=== Summary ===\n");
  console.log("Agent Key                  | Agent ID                                | Status");
  console.log("---------------------------+-----------------------------------------+--------");
  for (const [key, val] of Object.entries(results)) {
    const id = val.agentId || "N/A";
    const status = val.agentId ? "OK" : "FAILED";
    console.log(`${key.padEnd(27)}| ${id.padEnd(40)}| ${status}`);
  }

  const successCount = Object.values(results).filter((v) => v.agentId).length;
  console.log(`\n${successCount}/${agents.length} agents created successfully.`);

  if (successCount > 0) {
    console.log("\nRestart your dev server to use the new agent IDs:");
    console.log("  Kill the server, then run: npx next dev\n");
  }
}
